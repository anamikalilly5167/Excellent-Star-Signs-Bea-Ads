...
SMTP_EMAIL = "excellentstarsigns@gmail.com"
SMTP_APP_PASSWORD = "pqpbvelpgfhmwcde"  # Gmail App Password
...
def send_otp_email(to_email, otp):
    ...
    msg["Subject"] = "Password Reset OTP - Excellent Star Signs"
    msg["From"] = SMTP_EMAIL
    msg["To"] = to_email
...
def admin_forgot_password():
    ...
    conn.close()
    if not admin:
        return jsonify({"ok": False, "error": "No staff account found with that email."}), 404
    otp = str(random.randint(100000, 999999))
    otp_store[email] = {"otp": otp, "expires": datetime.utcnow() + timedelta(minutes=10)}
    try:
        send_otp_email(email, otp)
    except Exception:
        return jsonify({"ok": False, "error": "Could not send OTP email. Try again later."}), 500
